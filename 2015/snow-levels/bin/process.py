...
def get_lakelevels():
    ...
    metadata = lines[:5]
    print metadata
    mf = open('lakelevels_metadata.txt', 'w')
    mf.write('{}'.format(metadata))
    mf.close()

    # then delete them from file
    f.close()
    open('current_lakelevels.csv', 'w').writelines(lines[5:])
...
lakelist_url = 'http://www3.wcc.nrcs.usda.gov/reportGenerator/view_csv/customMultipleStationReport/monthly/state=%22OR%22,%22WA%22%20AND%20element=%22RESC%22%20AND%20outServiceDate=%222100-01-01%22%7Cname/POR_BEGIN,POR_END:M%7C' + str(month) + '/name,huc.huc'
