...
function ready(error, geography, resultsData, cities) {
  ...
  for (var i = 0; i < resultsData.length; i++) {
    if (resultsData[i].ap_race_number == "38002") {
      var repPres = resultsData[i].reporting_units;
    }
  }
  var repPresresultsData = repPres.reduce(function(result, county) {
  ...
  var states = svg.selectAll('path.state')
      ...
      .enter()
       ...
       .attr("class", function(d) {
             var inTheLead = repPresresultsData[( (d.properties.NAME).replace(/\s/g, ''))].unit_results[0].candidate;
             var reporting = repPresresultsData[( (d.properties.NAME).replace(/\s/g, ''))].unit_results[0].vote_total;
       ...
       .on("mousemove", function(d){
         var candidate = repPresresultsData[( (d.properties.NAME).replace(/\s/g, ''))].unit_results;
         return tooltip
           .html(
               ...
               "<table style='width:100%'>" +
                 ...
                 "</tr>" +
                 "<tr>" +
                   "<td>" + candidate[2].candidate + "</td>" +
                   "<td style='text-align:right;'>" + numberWithCommas(candidate[2].vote_total) + "</td>" +
                 "</tr>" +
               "</table>"
             )
             .style("top", function() {
